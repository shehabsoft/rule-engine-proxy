...
import com.emu.rule.proxy.config.KafkaProperties;
...
class RuleEngineProxyKafkaResourceIT {
    ...
    void setup() {
        ...
        RuleEngineProxyKafkaResource kafkaResource = new RuleEngineProxyKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/rule-engine-proxy-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/rule-engine-proxy-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
